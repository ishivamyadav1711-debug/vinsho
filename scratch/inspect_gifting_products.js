...
console.log("\nSearching seed / products json / database for matching product records:");
targetList.forEach(item => {
  const normName = item.name.replace(/—/g, '-').replace(/×/g, 'x').replace(/"/g, '');
  const baseName = item.name.split('—')[0].trim();
  
  const seedMatches = seed.products.filter(p => p.name && p.name.toLowerCase().includes(baseName.toLowerCase()));
  const jsonMatches = prodsJson.filter(p => p.name && p.name.toLowerCase().includes(baseName.toLowerCase()));
  const dbMatches = db ? db.prepare('SELECT id, name, slug, is_purchasable FROM products WHERE name LIKE ?').all(`%${baseName}%`) : [];

  console.log(`\n[#${item.id}] ${item.name}`);
  console.log(`  Seed matches (${seedMatches.length}):`, seedMatches.map(s => ({ slug: s.slug, name: s.name, image: s.image })));
  console.log(`  DB matches (${dbMatches.length}):`, dbMatches);
});
