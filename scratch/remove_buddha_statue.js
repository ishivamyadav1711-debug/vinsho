...
const targetSlug = 'laughing-buddha-statue';
const targetName = 'Laughing Buddha Statue';

console.log('--- Step 1: Remove Laughing Buddha Statue from SQLite database ---');
try {
  ...
  if (product) {
    ...
    const delVariants = db.prepare('DELETE FROM product_variants WHERE product_id = ?').run(product.id);
    console.log('Deleted product variants:', delVariants.changes);

    // Delete product images
    const delImages = db.prepare('DELETE FROM product_images WHERE product_id = ?').run(product.id);
