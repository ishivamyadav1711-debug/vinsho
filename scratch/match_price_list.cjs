...
officialList.forEach((item) => {
  ...
  // Extract combo number if present
  ...
  let matches = seedData.products.filter(p => {
    ...
    if (pNorm === itemNorm) return true;

    // Word overlap match
    const itemWords = itemNorm.split(' ').filter(w => w.length > 1 && w !== 'combo' && w !== comboNum);
    const pWords = pNorm.split(' ').filter(w => w.length > 1);

    const matchedWords = itemWords.filter(w => pNorm.includes(w));
    return matchedWords.length >= Math.max(1, itemWords.length - 1);
  });

  console.log(`\nItem #${item.id}: "${item.name}" (Target Price: ₹${item.price})`);
  if (matches.length === 1) {
    const p = matches[0];
    console.log(`  ✓ MATCH: [slug: ${p.slug}] "${p.name}" (Current Price: ₹${p.sellingPrice || p.price})`);
