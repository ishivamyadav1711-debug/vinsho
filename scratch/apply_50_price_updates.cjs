...
priceUpdates.forEach((item) => {
  const product = seedData.products.find(p => p.slug === item.slug);
  if (product) {
    product.sellingPrice = item.price;
    product.mrp = Math.round(item.price * 1.25);
    if (product.variants && Array.isArray(product.variants)) {
      product.variants.forEach(v => {
        v.sellingPrice = item.price;
        v.mrp = Math.round(item.price * 1.25);
      });
    }
    updatedCount++;
    console.log(`✓ [Item #${item.id}] Updated "${product.name}" (${product.slug}) -> sellingPrice: ₹${product.sellingPrice}, mrp: ₹${product.mrp}`);
  } else {
    console.error(`❌ COULD NOT FIND PRODUCT FOR SLUG: ${item.slug}`);
  }
});

if (updatedCount === 50) {
  fs.writeFileSync(seedFilePath, JSON.stringify(seedData, null, 2), 'utf8');
  console.log(`\nSUCCESS: Updated all 50 products in vinsho-commerce-seed.json`);
} else {
  console.error(`\nERROR: Only updated ${updatedCount}/50 products! Aborting save.`);
  process.exit(1);
}
