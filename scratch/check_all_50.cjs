...
console.log("=== CHECKING MATCHES IN DATABASE AND SEED DATA FOR ALL 50 PRODUCTS ===");

let matchedInSeed = 0;
let matchedInDb = 0;

list.forEach(item => {
  const normName = item.name.replace(/—/g, '-').replace(/×/g, 'x');
  const seedP = seed.products.find(p => p.name === item.name || p.name === normName);
  const dbP = db ? db.prepare('SELECT p.id, p.name, p.slug, pi.url FROM products p LEFT JOIN product_images pi ON p.id = pi.product_id AND pi.is_primary = 1 WHERE p.name = ? OR p.name = ?').get(item.name, normName) : null;
  
  if (seedP) matchedInSeed++;
  if (dbP) matchedInDb++;

  console.log(`[#${item.id}] "${item.name}" -> Seed: ${seedP ? 'EXISTS (' + seedP.image + ')' : 'MISSING'}, DB: ${dbP ? 'EXISTS (' + dbP.url + ')' : 'MISSING'}`);
});

console.log(`\nTotals: Matched in Seed = ${matchedInSeed}/50, Matched in DB = ${matchedInDb}/50`);
