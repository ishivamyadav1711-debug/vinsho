...
console.log('=== DRY RUN VERIFICATION OF 52 CANDLE CATALOG UPDATES ===');

const updated52Map = images52.map(img => {
  const normSlug = img.file.replace(/^\d+-/, '').replace(/\.jpg$/, '');
  const imagePath = `/products/candles/${img.file}`;

  let existing = seedData.products.find(p => p.slug === normSlug || p.slug === normSlug.replace(/-and-/g, '-'));
  if (!existing && img.id === 19) {
    existing = seedData.products.find(p => p.slug === 'diamond-jar-candle-gift');
  }
  ...
  return {
    ...
    file: img.file,
    name: img.name,
    targetSlug: existing ? existing.slug : normSlug,
    imagePath,
    isNewProduct: !existing,
    existingPrice: existing ? (existing.price || existing.sellingPrice || null) : null
  };
});

console.log(`Total 52 entries processed: ${updated52Map.length}`);
console.log(`Existing products being updated with exact image: ${updated52Map.filter(x => !x.isNewProduct).length}`);
console.log(`New catalog products being created: ${updated52Map.filter(x => x.isNewProduct).length}`);
