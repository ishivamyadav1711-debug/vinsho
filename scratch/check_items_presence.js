...
userList.forEach((item, index) => {
  ...
  // If no exact norm match, check partial / fuzzy containment match
  if (!match) {
    match = websiteNames.find(w => {
      // e.g. T.T. Planter vs Test Tube Planter or variation
      return w.norm.includes(normItem) || normItem.includes(w.norm);
    });
  }
...
console.log("=== PRESENT ===");
present.forEach(p => console.log(`${p.index}. ${p.item} => matched: "${p.matchedName}" (${p.slug})`));
